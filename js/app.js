...
var Footer = {
	...
	widgets: {
		...
		instagram: {
			...
			photoStreamEl: $('.photo-stream'),
			firstItemRevealed: false,
			
			init: function() {

				var createPhotoElement = function(photo, i) {
					var img = $('<img>').addClass('instagram-image').attr('src', photo.images.thumbnail.url),
						a = $('<a>').attr({
							id: photo.id,
							class: (i == 0 ? 'first-item' : ''),
							target: '_blank',
							href: photo.link
						}).append(img);
					return a;
				...
				var didLoadInstagram = function(event, response) {
					var that = this;
					$.each(response.data, function(i, photo) {
						$(that).append(createPhotoElement(photo, i));
					});
				};
				
				this.photoStreamEl.instagram({
					clientId: 'efd2f9067921439fb1febc340c0fee45',
					hash: 'airnauts',
					count: 12,
	...
	updateBodySize: function() {
		if (!Modernizr.touch) {
			...
			if (window.innerWidth > tabletWidth) {
				height = ContentSections.offsetTop + window.innerHeight * ContentSections.el.children('.section').length + Footer.el.outerHeight();
			}
				...
				if (rubberBand) {
					...
					if (!Modernizr.touch) {
						...
						if (direction == 'next') {
							...
							TweenMax.to(proj.find('.about'), .7, { x: -window.innerWidth/2, ease: Sine.easeOut });
							...
							setTimeout(function() {
								TweenMax.to(Work.details.itemsEl, .7, { x: xVal, ease: Sine.easeOut, onComplete: function() {
									...
									TweenMax.to(proj.find('.about, .download-bar, .short-info'), .01, { x: 0 });
								}});
								TweenMax.to(Work.details.itemsHelperEl, .7, { x: xVal, ease: Sine.easeOut });
							}, 100);
							
							TweenMax.fromTo(proj2.find('.about'), .7, { x: window.innerWidth/2 }, { x: 0, ease: Sine.easeOut, delay: .5 });
							...
							TweenMax.to(proj.find('.about'), .7, { x: window.innerWidth, ease: Sine.easeOut, delay: .1 });
